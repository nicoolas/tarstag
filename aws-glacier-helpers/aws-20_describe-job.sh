...
vault="$1"
in=$(f_get_filepath "${vault}" "$file_job_inventory")
out=$(f_get_filepath "${vault}" "$file_job_description")
...
set -x
aws glacier describe-job --vault-name $vault --account-id - --job-id="$job_id" | tee $out
